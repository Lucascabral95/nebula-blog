...
export async function GET() {
  try {
    const posts = await PostDAO.getPostsWithoutPopulate();
...
export async function PUT(req) {
  try {
    ...
    if (!id) {
      return NextResponse.json(
        { error: "El ID del post es obligatorio" },
        { status: 400 }
      );
    }
    ...
    if (result.modifiedCount === 0) {
      return NextResponse.json(
        {
          error:
            "No se encontró el post o no se realizó ninguna modificación",
        },
        { status: 404 }
      );
    }

    const postActualizado = await PostDAO.getPostById(id);

    return NextResponse.json(
      {
        message: "Like dado exitosamente",
        cantidadDeLikes: postActualizado?.likes || 0,
      },
      { status: 200 }
    );
  } catch (error) {
